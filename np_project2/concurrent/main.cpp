...
int main(int argc, char *argv[]) {
  ...
  ftruncate(shm_userpipe_fd,
            sizeof(int) * (BUFFER_SIZE + 1) * (BUFFER_SIZE + 1) * 2);
  ftruncate(shm_login_fd, sizeof(int));

  user_pipe = (int(*)[CLIENT_NUMBER + 1][2])
      get_shared_memory(shm_userpipe_fd, sizeof(int) * (CLIENT_NUMBER + 1) *
                                             (CLIENT_NUMBER + 1) * 2);
  share_message =
      (char *)get_shared_memory(shm_message_fd, sizeof(char) * BUFFER_SIZE);
  client_info = (client *)get_shared_memory(
      shm_client_info_fd, sizeof(client) * (CLIENT_NUMBER + 1));
  login_cnt = (int *)get_shared_memory(shm_login_fd, sizeof(int));

  memset(user_pipe, 0,
         sizeof(int) * (CLIENT_NUMBER + 1) * (CLIENT_NUMBER + 1) * 2);
