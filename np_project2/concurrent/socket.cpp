...
void broadcast(pid_t client_pid, int option, char *_message, int from_id,
               int to_id) {
  ...
  case CHANGENAME:
    ...
    break;
  case RECEIVEPIPE:
    sprintf(share_message,
            "*** %s (#%d) just received from %s (#%d) by '%s' ***\n",
            client_ptr->name, client_ptr->id, from_client->name,
            from_client->id, _message);
    break;
  case SENDPIPE:
