...
static int checkname(int clientfd, char *name)
{
    ...
    {
        if ((client_info[i].client_fd != clientfd) && (!strcmp(client_info[i].name, name)))
        {
            ret = -1;
        }
    }
...
static void process_handler(string _command, int *cnt_line, int cross_line_pipe_table[][2], int clientfd, bool is_userpipe)
{
...
int my_shell(int clientfd) {
	...
	else if (check_user_pipe(command))
	{
        ...
        process_handler(buff, &(client_ptr->cnt_line), client_ptr->cross_line_pipe_table, clientfd, true);
	}
    else
    {
        process_handler(buff, &(client_ptr->cnt_line), client_ptr->cross_line_pipe_table, clientfd, false);
    }
...
static void env_handler(vector<string> command, int clientfd)
{
    ...
    if (command[0] == "setenv")
    {
        ...
        strncpy(client_ptr->path, strdup(command[2].c_str()), strlen(strdup(command[2].c_str())));
    }
    else
    {
