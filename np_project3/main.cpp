...
public:
  echo_session(tcp::socket socket) : _socket(move(socket)) {}
...
private:
  void send_cgi_web() {
    ...
    sprintf_s(temp, "<!DOCTYPE html>"
                    ...
                    "css?family=Source+Code+Pro\" rel =\"stylesheet\"/>"
    ...
    sprintf_s(temp, "<form action =\"console.cgi\" method = \"GET\"><table "
                    "class =\"table mx-auto bg-light\" style =\"width: "
                    "inherit\"><thead class =\"thead-dark\"><tr>"
                    "<th scope =\"col\">#</th><th scope =\"col\">Host</th><th "
                    "scope =\"col\">Port</th><th scope =\"col\">Input "
                    "File</th></tr></thead><tbody>");

    strncat_s(s_buffer, temp, strlen(temp));

    string test_file[10] = {"t1.txt", "t2.txt", "t3.txt", "t4.txt", "t5.txt",
                            "t6.txt", "t7.txt", "t8.txt", "t9.txt", "t10.txt"};

    string final_test_menu = "";

    for (int i = 0; i < 10; ++i) {
      final_test_menu = final_test_menu + "<option value=\"" + test_file[i] +
                        "\">" + test_file[i] + "</option>";
    }
    string domain_name = "cs.nctu.edu.tw";

    string host[10] = {"nplinux1", "nplinux2", "nplinux3", "nplinux4",
                       "nplinux5", "npbsd1",   "npbsd2",   "npbsd3",
                       "npbsd4",   "npbsd5"};

    string host_menu[10];
    for (int i = 0; i < 9; i++) {
      host_menu[i] = "<option value=\"" + host[i] + "." + domain_name + "\">" +
                     host[i] + "</option>";
    }
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      send_buffer = send_buffer + "<select name =\"h" + std::to_string(i) +
                    "\" class =\"custom-select\">";
      ...
      send_buffer = send_buffer + "<input name =\"p" + std::to_string(i) +
                    "\" type =\"text\" class =\"form-control\" size =\"5\" />";
      send_buffer += "</td><td>";
      send_buffer = send_buffer + "<select name =\"f" + std::to_string(i) +
                    "\" class =\"custom-select\">";
    ...
    sprintf_s(temp, "<tr><td colspan =\"3\"></td><td><button type=\"submit\" "
                    "class = \"btn btn-info "
                    "btn-block\">Run</button></td></tr></tbody></table></"
                    "form></body></html>");

    strncat_s(s_buffer, temp, strlen(temp));

    _socket.async_send(
        boost::asio::buffer(s_buffer, strlen(s_buffer)),
        [self, this](const boost::system::error_code ec, size_t w_len) {
          if (!ec) {
            // cout << "success" << endl;
          } else {
  ...
  void do_write(char *_buffer) {
    _socket.async_send(
        boost::asio::buffer(_buffer, strlen(_buffer)),
        [this](const boost::system::error_code ec, size_t w_len) {
          if (!ec) {
            // success
          } else {
            // error
  ...
  void do_service() {
    ...
    _socket.async_send(
        boost::asio::buffer(status_code, 50),
        [this, self](const boost::system::error_code ec, size_t write_length) {
          if (!ec) {
            if (!strncmp(cgi_name, "/panel.cgi", strlen("/panel.cgi"))) {
              send_cgi_web();
            } else if (!strncmp(cgi_name, "/console.cgi",
                                strlen("/console.cgi"))) {
              int server_index = 1;

              // tcp_client client(io_service_for_client)[SERVER_SIZE + 1]; //
              // initialize tcp_client client[SERVER_SIZE + 1]; tcp_client
              // client[SERVER_SIZE + 1];
  ...
  void print_html_web() {
    ...
    str += "<meta charset=\"UTF-8\"><title>NP Project 3 Console</title><link "
           "rel=\"stylesheet\" "
    ...
    str += "<style>* { font-family: 'Source Code Pro', monospace; font-size: "
           "1rem !important}body {background-color: #212529;}pre {color: "
           "#cccccc;}b{color: #ffffff;}</style>";
    str += "</head><br><body><br><table class=\"table table-dark "
           "table-bordered\"><br><thead><br><tr><br>";
    ...
    for (int i = 1; i <= SERVER_SIZE; ++i) {
      if (server[i].enable) {
        sprintf_s(temp, "<th scope=\"col\">%s:%s</th><br>",
                  server[i].domain_name, server[i].port);
        strncat(s_buffer, temp, strlen(temp));
        memset(temp, 0, sizeof(temp));
      }
    }
    ...
    for (int i = 1; i <= SERVER_SIZE; ++i) {
      if (server[i].enable) {
        sprintf_s(temp, "<td><pre id=\"s%d\" class=\"mb-0\"></pre></td><br>",
                  server_index);
        strncat(s_buffer, temp, strlen(temp));
        memset(temp, 0, sizeof(temp));
        server_index++;
...
public:
  friend class tcp_client; // declare that tcp_client can access members in
                           // echo_session.
...
private:
  ...
  public:
    tcp_client()
        : _resolver(io_service_for_client), _socket(io_service_for_client) {}
  ...
  private:
    ...
    void do_resolve(server_info &_server, echo_session &http_obj) {
      ...
      _resolver.async_resolve(query, [&](const boost::system::error_code ec,
    ...
    void output_shell(char *_message, server_info &_server,
                      echo_session &http_obj) {
      ...
      while (getline(ss, item, '\n')) {
        ...
        sprintf_s(temp,
                  "<script>document.getElementById(\'s%d\').innerHTML += "
                  "\'%s\';</script>\n",
                  _server.server_index, buffer);

        http_obj._socket.async_send(
            boost::asio::buffer(temp, strlen(temp)),
            [this](const boost::system::error_code ec, size_t write_len) {
              if (!ec) {
                // success.
              } else {
    ...
    void send_command(server_info &_server, echo_session &http_obj) {
      ...
      http_obj._socket.async_send(
          boost::asio::buffer(temp, strlen(temp)),
          [this](const boost::system::error_code ec, size_t write_length) {
            if (!ec) {
              // success
            } else {
...
public:
  tcp_server(unsigned short port)
      : _socket(global_io_service), _acceptor(global_io_service),
