...
static int fire_wall(uint8_t *req)
{
    ...
    while (fgets(buffer, BUFFER_SIZE, fp))
    {
        ...
        else if (ip[0] == req[4] && ip[1] == 0 && ip[2] == 0 && ip[3] == 0 && cnt == 3)
        {
            permit = 1;
        }

        else if (ip[0] == req[4] && ip[1] == req[5] && ip[2] == 0 && ip[3] == 0 && cnt == 2)
        {
            permit = 1;
        }

        else if (ip[0] == req[4] && ip[1] == req[5] && ip[2] == req[6] && ip[3] == 0 && cnt == 1)
        {
            permit = 1;
        }

        else if (ip[0] == req[4] && ip[1] == req[5] && ip[2] == req[6] && ip[3] == req[7] && cnt == 0)
        {
            permit = 1;
        }
...
static int do_bindmode(int source_socket, uint8_t *socks4_reply)
{
    ...
    for (int i = 4; i < 8; ++i) socks4_reply[i] = 0x00;
